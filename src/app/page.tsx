...
export default function Home() {
  ...
  // Load existing votes for this voter from Supabase
  useEffect(() => {
    if (!voterName || voterName.length < 2) return;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        allVotes
          ...
          .forEach((v) => {
            myVotes[v.example_id] = v.approved;
          });
        setVotes(myVotes);
      } catch {
        // Supabase not configured yet, use local state
      }
    }, 500);
  ...
  const handleVote = useCallback(
    async (exampleId: string, moduleId: number, moduleName: string, approved: boolean) => {
      ...
      try {
        ...
        // Supabase not configured, votes saved locally only
      }
    },
    [voterName, votes]
  );
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ paddingBottom: "60px" }}>
      ...
      >
        ...
        />
        <div
          className="absolute -bottom-16 -left-8 w-56 h-56 rounded-full opacity-5"
          style={{ background: "#fff" }}
        />
      </div>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {sectors.map((sector) => {
            ...
            const count =
              sector === "Todos"
                ? modulosCaos.length
                : modulosCaos.filter((m) => m.sector === sector).length;
      ...
      <div className="max-w-[1400px] mx-auto w-full px-4 sm:px-6 flex flex-col gap-6 pb-8">
        ...
        {filteredModules.length === 0 && (
          <div className="text-center py-16">
            <p className="text-lg font-semibold" style={{ color: "var(--foreground-muted)" }}>
      ...
      {/* Voting Panel (fixed bottom) */}
      {voterName.length >= 2 && <VotingPanel votes={votes} voterName={voterName} />}
    </div>
  );
}
